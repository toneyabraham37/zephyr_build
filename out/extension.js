...
function activate(context) {
    console.log('MCHP Build Extension Activated.');
    installMissingExtensions();
    const disposableTerminal = vscode.commands.registerCommand('mchpbuild.openTerminal', () => {
    ...
    const disposableBuild = vscode.commands.registerCommand('mchpbuild.runBuild', (args) => {
    ...
    const disposableFlash = vscode.commands.registerCommand('mchpbuild.runFlash', () => {
        const terminal = getOrCreateTerminal();
        terminal.sendText('west flash');
    });
    const disposableSelectProject = vscode.commands.registerCommand('mchpbuild.selectProject', async () => {
        ...
        if (folders && folders.length > 0) {
            ...
            vscode.commands.executeCommand('setContext', 'mchpbuild.selectedProject', selectedPath);
            if (currentWebview) {
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(message => {
            if (message.command === 'openTerminal') {
                vscode.commands.executeCommand('mchpbuild.openTerminal');
            }
            else if (message.command === 'selectProject') {
                vscode.commands.executeCommand('mchpbuild.selectProject');
            }
            else if (message.command === 'runBuild') {
                vscode.commands.executeCommand('mchpbuild.runBuild', message);
            }
            else if (message.command === 'runFlash') {
                vscode.commands.executeCommand('mchpbuild.runFlash');
            }
        });
    }
    getHtml() {
