  ...
  vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        vscode.window.showInformationMessage(
            installedCount > 0 ?
                `Installed ${
                    installedCount} missing extensions. Reload VS Code if needed.` :
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposableTerminal =
      vscode.commands.registerCommand('mchpbuild.openTerminal', () => {
        const terminal =
            vscode.window.terminals.find(t => t.name === 'Zephyr Terminal');
        if (terminal) {
  ...
  const disposableBuild =
      vscode.commands.registerCommand('mchpbuild.runBuild', (args) => {
        ...
        terminal.sendText(`west build ${cleanOption}-b ${boardName}${
                              pathOption}${configOption}`
                              .trim());
      });

  const disposableFlash =
      vscode.commands.registerCommand('mchpbuild.runFlash', () => {
        const terminal = getOrCreateTerminal();
        terminal.sendText('west flash');
      });

  const disposableSelectProject =
      vscode.commands.registerCommand('mchpbuild.selectProject', async () => {
        ...
        if (folders && folders.length > 0) {
          ...
          vscode.commands.executeCommand(
              'setContext', 'mchpbuild.selectedProject', selectedPath);
          if (currentWebview) {
...
class BuildViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'mchpbuildView';
